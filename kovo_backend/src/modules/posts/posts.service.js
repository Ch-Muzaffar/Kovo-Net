...
async function createPost(userId, data) {
  // Step 1: Concurrently run content moderation check and fetch creator profile
  const [moderationResult, creatorRes] = await Promise.all([
    moderateText(data.title + ' ' + data.body),
    supabaseAdmin
      .from('user_profiles')
  ...
  const postWithCreator = { ...post, tags: data.tags || [], creator };

  // Step 3: Concurrently perform tag insertion, cache invalidation, and websocket broadcasting in the background
  (async () => {
    try {
      ...
      logger.error('Failed to insert post tags', { postId: post.id, error: err.message });
    }

    try {
      cache.invalidatePattern('feed:');
    } catch (err) {
      logger.error('Failed to invalidate feed cache', { error: err.message });
    }
    ...
    try {
      ...
      logger.error('Failed to broadcast new post via websocket', { error: err.message });
    }
  })();
...
async function getPost(postId, userId) {
  // PERF: Fetch post and tags concurrently
  ...
  if (postRes.error || !post) throw new NotFoundError('Post not found');
  if (post.is_hidden && post.user_id !== userId) throw new NotFoundError('Post not found');

  return { ...post, tags: tagsRes.data || [] };
}
...
async function updatePost(postId, userId, data) {
  ...
  if (data.tags) {
    // PERF: Fire-and-forget tag update in background (non-critical for response)
    (async () => {
      try {
        ...
        logger.error('Failed to update post tags', { postId, error: err.message });
      }
    })();
  }

  // PERF: Fire-and-forget cache invalidation
  cache.invalidatePattern('feed:');
  return post;
}
...
async function deletePost(postId, userId) {
  ...
  const { reclaimPointsForPost } = require('../ledger/ledger.service');
  await reclaimPointsForPost(postId);
...
async function getUserPosts(targetUserId, _requesterId, { cursor, pageSize }) {
  ...
  return { data: items, pagination: { hasMore, total: data.length, nextCursor } };
}
