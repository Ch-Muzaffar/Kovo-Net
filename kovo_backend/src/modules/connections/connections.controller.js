...
class ConnectionsController {
  ...
  static async getReceivedPending(req, res, next) {
    try {
      const pending = await ConnectionsService.getPendingRequests(req.user.id);
      res.status(200).json({ success: true, data: pending });
    } catch (error) { next(error); }
  ...
  static async getSentPending(req, res, next) {
    try {
      const pending = await ConnectionsService.getSentPendingRequests(req.user.id);
      res.status(200).json({ success: true, data: pending });
    } catch (error) { next(error); }
  }
  ...
  static async accept(req, res, next) {
    try {
      ...
      const connection = await ConnectionsService.respondRequest(req.user.id, connectionId, 'accept');
      res.status(200).json({ success: true, data: connection });
    } catch (error) { next(error); }
  }
  ...
  static async deleteRequest(req, res, next) {
    try {
      const { connectionId } = req.params;
      const result = await ConnectionsService.deleteConnectionRequest(req.user.id, connectionId);
  ...
  static async remove(req, res, next) {
    try {
      const { connection_id } = req.body;
      const result = await ConnectionsService.removeConnection(req.user.id, connection_id);
      res.status(200).json({ success: true, data: result });
    } catch (error) { next(error); }
  }
}
