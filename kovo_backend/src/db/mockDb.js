...
const tables = {
  ...
  user_profiles: [], // { id, username, email, first_name, last_name, date_of_birth, country, city, profession, user_type, avatar_url, bio, is_profile_complete, tos_accepted, tos_accepted_at, banned, banned_reason, points }
...
class MockQuery {
  ...
  gte(field, value) {
    this._gteField = field;
    this._gteValue = value;
    return this;
  }

  lte(field, value) {
    this._lteField = field;
    this._lteValue = value;
    return this;
  }
  ...
  _projectFields(row) {
    ...
    let current = '';
    let depth = 0;
    for (let i = 0; i < this._selectFields.length; i++) {
      const char = this._selectFields[i];
      if (char === '(') depth++;
      else if (char === ')') depth--;
      if (char === ',' && depth === 0) {
    ...
    for (const part of parts) {
      if (part === '*') {
        ...
        // or "sender:user_profiles!connections_sender_id_fkey(id, first_name, last_name, avatar_url, bio)"
        const [alias, rest] = part.split(':');
        const tablePart = rest.match(/^([\w!]+)\((.+)\)$/);
        if (tablePart) {
          let joinTable = tablePart[1];
          if (joinTable.includes('!')) {
            joinTable = joinTable.split('!')[0];
          }
          const joinFields = tablePart[2].split(',').map(s => s.trim());
          
          let foreignKeyVal;
          if (alias === 'sender') {
            foreignKeyVal = row.sender_id;
          } else if (alias === 'receiver') {
            foreignKeyVal = row.receiver_id;
          } else {
            foreignKeyVal = row.user_id || row.id;
          }
          
          const joinRow = tables[joinTable]?.find(r => r.id === foreignKeyVal) || null;
          if (joinRow) {
  ...
  async _execute() {
    ...
    try {
      ...
      if (this._lteField) {
        rows = rows.filter(r => r[this._lteField] <= this._lteValue);
      }
      if (this._gteField) {
        rows = rows.filter(r => r[this._gteField] >= this._gteValue);
      }
      if (this._order) {
        rows.sort((a, b) => {
...
function createMockUserClient(_token) {
